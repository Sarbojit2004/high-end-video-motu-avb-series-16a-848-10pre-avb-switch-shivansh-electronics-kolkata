...
// Type system ported from the Neumann REEL's own committed woff2 files (Sec 8a).
...
for (const f of FONTS) {
  ...
  if (!existsSync(src)) throw new Error(`Missing ported font ${f}. Expected the Neumann reel repo at ${NEUMANN} — Section 8a requires the type system be inherited, not invented.`);
  copyFileSync(src, resolve(PROJ, "public/fonts", f));
}
console.log(`fonts: ${FONTS.length} ported from neumann-tlm-107-new`);
