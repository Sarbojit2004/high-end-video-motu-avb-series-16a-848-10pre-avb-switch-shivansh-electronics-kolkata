...
// COMPRESSED REEL: 88 s. Same Path A / Path B blend as the master reel, with
// the body collapsed to ONE track (ETERNITY) — at 88 s a track change every
// ~15 s reads as restlessness rather than structure. The longest single-track
// span is the 62 s body, comfortably inside ETERNITY's 142.7 s, so nothing
// loops. Thresholds stay at the three-part reels' values: they are the
// stricter test and there is no reason to relax them.
const NEED = {
...
if (sfx.length < 34) fail(`expected >=34 SFX files for this reel's firing rate, found ${sfx.length}`);
...
// Placeholder VO slot
console.log("\nVO SLOT");
const vo = resolve(PROJ, "public/vo/voiceover-compressed-reel.mp3");
console.log(`   ${existsSync(vo) ? "present" : "absent (placeholder — narration recorded separately)"}: public/vo/voiceover-compressed-reel.mp3`);
