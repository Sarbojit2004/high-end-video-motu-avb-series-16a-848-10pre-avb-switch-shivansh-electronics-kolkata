...
const STAGE = resolve(PROJ, ".zipstage/motu-avb-compressed-reel");
const OUT = resolve(PROJ, "out/motu-avb-compressed-reel-project.zip");
...
for (const f of ["src", "scripts", "package.json", "tsconfig.json", "remotion.config.ts",
                 ...
                 "ASSET_COVERAGE_COMPRESSED_REEL.md", "BRANDING_CADENCE_COMPRESSED_REEL.md"]) {
...
execFileSync("zip", ["-r", "-q", OUT, "motu-avb-compressed-reel"], { cwd: resolve(PROJ, ".zipstage") });
rmSync(resolve(PROJ, ".zipstage"), { recursive: true, force: true });
