...
const raw = JSON.parse(readFileSync("../master-reel/scripts/inventory-raw.json", "utf8"));
const classified = JSON.parse(readFileSync("../reels/asset-manifest.json", "utf8"));
// The master reel's own curated manifest, to assert this selection is a
// SUBSET of it rather than an independent re-curation (Section 0a).
const MASTER = JSON.parse(readFileSync("../master-reel/asset-manifest.json", "utf8"));
const MASTER_IDX = new Set(MASTER.filter((a) => a.product !== "Brand").map((a) => a.idx));
...
for (const idx of [...new Set([...CURATED, ...LOGOS])]) {
  ...
  if (a.product !== "Brand" && !MASTER_IDX.has(idx))
    throw new Error(`image ${idx} is NOT in the master reel's curated set — Section 0a says this reel draws from that subset, so a pull-back must be declared deliberately.`);
...
console.log(`drawn from the master reel's curated ${MASTER_IDX.size}, itself selected from ${classified.filter((a) => a.product !== 'Brand').length} unique product images (${raw.length} files, ${raw.length - classified.length} pixel-identical duplicates folded)`);
