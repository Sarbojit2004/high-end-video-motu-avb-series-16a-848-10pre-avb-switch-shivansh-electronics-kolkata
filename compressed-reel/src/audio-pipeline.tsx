...
export function accents(beats: Beat[], startsArr: number[], totalFrames: number) {
  ...
  beats.forEach((b, i) => {
    ...
    /**
     * ECOSYSTEM MONTAGE. The master reel's accent map never covered this kind,
     * because there it was 4 beats of 42 and the picture carried them alone.
     * Here it is the reel's spine, and a beat where three images each take the
     * frame and hand it on needs the hand-offs articulated — otherwise the
     * primary technique is the one part of the reel with no sound of its own.
     *
     * One detent on each solo hand-off, rotating through the palette so a
     * 3- or 4-member beat never repeats inside itself, then an air on the
     * assembly, where the members settle into the shared composition.
     */
    if (b.kind === "ecosystemMontage") {
      const n = b.images.length;
      const hold = b.soloHold ?? Math.max(14, Math.min(Math.floor((len * 0.62) / n), 40));
      const DETENTS = ["encoder-detent", "encoder-detent-mid", "encoder-detent-hi",
                       "encoder-detent-lo", "encoder-detent-top"];
      // Six montage beats x (n-1) hand-offs is a short cycle, so the tick list
      // is ordered to guarantee every voice in it is reached rather than left
      // stranded by the index arithmetic.
      const TICKS = ["counter-tick-hi", "counter-tick", "relay-tick", "relay-tick-hi",
                     "relay-tick-lo", "relay-tick-top", "counter-tick-lo"];
      for (let k = 1; k < n; k++) {
        out.push({ at: start + k * hold, sfx: DETENTS[(i + k) % DETENTS.length], gain: 0.15 });
        out.push({ at: start + k * hold + 3, sfx: TICKS[(i * 2 + k) % TICKS.length], gain: 0.09 });
      }
      const AIRS = ["panel-air-tight", "panel-air", "panel-air-soft", "panel-air-hi"];
      const PINGS = ["avb-ping-alt", "avb-ping-warm", "avb-ping-lo", "avb-ping-hi", "avb-ping-mid"];
      out.push({ at: start + n * hold, sfx: AIRS[i % AIRS.length], gain: 0.18 });
      b.images.forEach((_, k) =>
        out.push({ at: start + n * hold + 6 + k * 4,
                   sfx: PINGS[(i + k) % PINGS.length], gain: 0.1 }));
    }
    ...
    if (b.kind === "dataFlow") {
      out.push({ at: start + 10, sfx: "data-stream", gain: 0.15 });
      out.push({ at: start + 22, sfx: "data-stream-short", gain: 0.18 });
      out.push({ at: start + Math.round(len * 0.42), sfx: "data-stream-long", gain: 0.12 });
      out.push({ at: start + Math.round(len * 0.58), sfx: "rj45-snap-soft", gain: 0.2 });
      out.push({ at: start + Math.round(len * 0.78), sfx: "gptp-lock", gain: 0.15 });
    }
