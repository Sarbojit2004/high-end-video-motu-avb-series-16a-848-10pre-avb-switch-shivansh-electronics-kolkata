// Design tokens — MOTU AVB Series ecosystem COMPRESSED REEL (portrait, light).
//
// PROVENANCE (Section 0/7): every value here is pulled UNCHANGED from the
// approved 298 s master reel's committed source on
// `claude/motu-avb-master-reel-ecosystem` — the closest same-scope precedent
// available (whole-ecosystem, curated selection, portrait). Nothing is
// re-derived, and nothing is taken from the Neumann project, which this build
// does not reference at all.
//
// The ONLY value changed from the source is VIDEO.durationInSeconds: 298 -> 88.
...
export const VIDEO = {
  ...
  get durationInFrames() {
    return Math.round(this.fps * this.durationInSeconds); // 2640
  },
} as const;
...
export const COMPRESSED = {
  ...
  out: "motu-avb-compressed-reel-ecosystem",
} as const;
