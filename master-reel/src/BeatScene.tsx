...
import { Plate, MacroReveal, PortSweep, Montage, Gimbal, EcosystemMontage } from "./components/Media";
import {
...
/**
 * Portrait layouts. Pulled from the approved three-part reel build's own
 * committed source (Section 0/3), extended with EcosystemMontageBeat.
 *
 * The usable box is 952 x 1520 starting at y=180 — the
...
/**
 * ECOSYSTEM MONTAGE BEAT — the master reel's own technique (Section 3).
 *
 * The heading states a claim that is true across the products shown; the media
 * slot gives each member a solo pass at full size before assembling them into
 * the composition that makes the cross-product point. The heading stays put
 * throughout, which is what welds the separate images into one assertion.
 */
const EcosystemMontageBeat: React.FC<{ b: Beat; d: number }> = ({ b, d }) => (
  <AbsoluteFill>
    <Frame style={{ paddingTop: SAFE.top + topClear(b), paddingBottom: SAFE.bottom + bottomClear(b) }}>
      <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0, gap: 22 }}>
        <TextBlock b={b} size={b.sub ? 68 : 76} />
        <div style={{ flex: 1, minHeight: 0, position: "relative" }}>
          <div style={{ position: "absolute", inset: 0 }}>
            <EcosystemMontage
              items={b.images.map((idx, i) => ({ idx, label: b.labels?.[i] }))}
              duration={d} cols={b.cols ?? 1} hold={b.soloHold}
            />
          </div>
        </div>
      </div>
...
export const BeatScene: React.FC<{ b: Beat }> = ({ b }) => {
  ...
  const bg = (
    <AbsoluteFill>
      <Ground bloom={COLORS.motuBlue} bloomStrength={bigLogo ? 0.03 : 0.07}
              flat={bigLogo} grid={b.kind === "ecosystemSplit" || b.kind === "ecosystemMontage"} />
      {b.kind === "ecosystemSplit" || b.kind === "ecosystemMontage" || b.kind === "dataFlow"
        ? <Motes seed={4} count={16} opacity={0.3} /> : null}
    </AbsoluteFill>
  );
  ...
  switch (b.kind) {
    ...
    case "ecosystemMontage": body = <EcosystemMontageBeat b={b} d={d} />; break;
  ...
  const enter = b.kind === "hook" ? "scaleIn"
    ...
    : b.kind === "ecosystemMontage" ? "pushLeft"
