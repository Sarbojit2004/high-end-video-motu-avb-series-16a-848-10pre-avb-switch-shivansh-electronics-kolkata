...
/**
 * ECOSYSTEM MONTAGE (Section 3) — NEW for the master reel.
 *
 * The prior two builds had Ecosystem Split: a triptych of the 16A/848/10pre
 * shown together specifically to assert peer equality BETWEEN INTERFACES. That
 * is a narrower point than this reel needs. Ecosystem Montage clubs images from
 * DIFFERENT products inside a single beat, for claims that are inherently
 * cross-product — three visibly different front panels under one shared-engine
 * assertion, or an interface's own Ethernet port beside the Switch's RJ-45
 * cluster, completing the "one network" claim by showing the physical
 * connection point on both sides.
 *
 * THE COMPLETENESS RULE IS NOT RELAXED BY CLUBBING. Each member gets its own
 * solo window — `hold` frames during which it is the only thing on screen, at
 * full size, through <Plate> (object-fit: contain) so it is whole and uncropped
 * — and only afterwards do the members settle into the shared composition that
 * makes the cross-product point. Clubbing images to make a point is never
 * licence to crop any one of them down to fit alongside the others.
 */
export const EcosystemMontage: React.FC<{
  items: { idx: number; label?: string }[];
  duration: number;
  /** Frames each member holds the frame alone before the group assembles. */
  hold?: number;
  /** Rows in the settled composition. Defaults to a single column. */
  cols?: number;
  gap?: number;
  style?: React.CSSProperties;
}> = ({ items, duration, hold, cols = 1, gap = 16, style }) => {
  const frame = useCurrentFrame();
  const n = items.length;
  // Split the beat: solo passes first, then the assembled composition. The solo
  // phase never takes more than 62% of the beat, so the cross-product point
  // still lands with time to read.
  const soloEach = hold ?? Math.max(14, Math.min(Math.floor((duration * 0.62) / n), 40));
  const soloEnd = soloEach * n;
  const assembling = frame >= soloEnd;
  const activeSolo = Math.min(n - 1, Math.floor(frame / soloEach));

  const rows = Math.ceil(n / cols);
  const cellW = (SAFE.contentW - gap * (cols - 1)) / cols;
  const rowH = Array.from({ length: rows }, (_, r) =>
    Math.max(...items.slice(r * cols, r * cols + cols).map((it) => cellW / ar(it.idx)))
  );
  const natural = rowH.reduce((a, b) => a + b, 0) + gap * (rows - 1);

  if (!assembling) {
    const it = items[activeSolo];
    const local = frame - activeSolo * soloEach;
    const t = ramp(local, 0, 9, EASE.out);
    const out = 1 - ramp(local, soloEach - 7, 7, EASE.inOut);
    return (
      <div style={{ position: "relative", width: "100%", height: "100%",
                    display: "flex", alignItems: "center", ...style }}>
        <div style={{ width: "100%", height: "100%", opacity: Math.min(t, out),
                      transform: `scale(${0.97 + t * 0.03})` }}>
          <Gimbal seed={it.idx} amount={0.5}>
            <Plate idx={it.idx} style={{ width: "100%", height: "100%" }} />
          </Gimbal>
        </div>
        {it.label ? (
          <div style={{
            position: "absolute", left: 0, bottom: 8, opacity: Math.min(t, out),
            ...micro(19, 800, "0.12em"), color: COLORS.ink,
            background: hexA(COLORS.paperLift, 0.94), padding: "7px 13px",
            borderRadius: 7, border: `1px solid ${COLORS.line}`,
          }}>{it.label}</div>
        ) : null}
        {/* solo progress — which member of the club is on screen right now */}
        <div style={{ position: "absolute", right: 0, bottom: 10, display: "flex", gap: 6 }}>
          {items.map((_, i) => (
            <span key={i} style={{
              width: i === activeSolo ? 22 : 8, height: 8, borderRadius: 4,
              background: i === activeSolo ? COLORS.motuBlue : hexA(COLORS.ink, 0.18),
            }} />
          ))}
        </div>
      </div>
  ...
  const a = ramp(frame - soloEnd, 0, 16, EASE.out);
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", ...style }}>
      <div style={{
        display: "grid",
        gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))`,
        gridTemplateRows: rowH.map((h) => `minmax(0, ${h.toFixed(2)}fr)`).join(" "),
        gap, width: "100%", height: natural, maxHeight: "100%",
      }}>
        {items.map((it, i) => (
          <div key={it.idx} style={{
            position: "relative", minWidth: 0, minHeight: 0,
            opacity: ramp(frame - soloEnd, i * 4, 14, EASE.out),
            transform: `translateY(${(1 - a) * 14}px)`,
          }}>
            <Gimbal seed={it.idx * 3} amount={0.4}>
              <Plate idx={it.idx} style={{ width: "100%", height: "100%" }} />
            </Gimbal>
            {it.label ? (
              <div style={{
                position: "absolute", left: 12, bottom: 10,
                ...micro(16, 800, "0.12em"), color: COLORS.ink,
                background: hexA(COLORS.paperLift, 0.94), padding: "5px 10px",
                borderRadius: 6, border: `1px solid ${COLORS.line}`,
              }}>{it.label}</div>
            ) : null}
          </div>
        ))}
