...
const built = [
  ...
  // The three-part reels needed 21 voices to cover 178 s without the palette
  // becoming audible as a loop. This reel runs 298 s at the same cadence — a
  // transition on every one of ~40 cuts plus accents inside each beat — so the
  // palette is widened to 28. Every addition is a new pitch or a new envelope
  // built from the same primitives, so the character is unchanged and the
  // sub-400 Hz constraint still holds for all of them.
