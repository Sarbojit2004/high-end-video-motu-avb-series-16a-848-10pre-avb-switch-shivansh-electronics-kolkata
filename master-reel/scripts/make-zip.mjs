...
const STAGE = resolve(PROJ, ".zipstage/motu-avb-master-reel");
const OUT = resolve(PROJ, "out/motu-avb-master-reel-project.zip");
...
for (const f of ["src", "scripts", "package.json", "tsconfig.json", "remotion.config.ts",
                 ...
                 "ASSET_COVERAGE_MASTER_REEL.md", "BRANDING_CADENCE_MASTER_REEL.md",
                 "VO_SCRIPT_MASTER_REEL_MOTU_AVB_ECOSYSTEM.md"]) {
...
families and the 28 synthesized SFX files, so those need no network and no
sibling repository.
...
execFileSync("zip", ["-r", "-q", OUT, "motu-avb-master-reel"], { cwd: resolve(PROJ, ".zipstage") });
rmSync(resolve(PROJ, ".zipstage"), { recursive: true, force: true });
