...
// MASTER REEL: 298 s, scored with the long-form's committed Path A / Path B
// blend — Mindscape bookends, each product segment scored from its own track.
// The longest single-track span here is a 50 s segment, so every track already
// covers its own span unlooped; the thresholds below stay at the three-part
// reels' values because they are the stricter test.
const NEED = {
...
if (sfx.length < 28) fail(`expected >=28 SFX files for a 298 s runtime, found ${sfx.length}`);
...
// Placeholder VO slot
console.log("\nVO SLOT");
const vo = resolve(PROJ, "public/vo/voiceover-master-reel.mp3");
console.log(`   ${existsSync(vo) ? "present" : "absent (placeholder — narration recorded separately)"}: public/vo/voiceover-master-reel.mp3`);
