...
export const buildSfxPlan = (segments: TimedSegment[], shots: Placed[], outroAt: number, fps: number): Cue[] => {
  ...
  for (const seg of segments) {
    ...
    if (seg.product === "p16a" || seg.product === "p848" || seg.product === "p10pre") {
      ...
      for (let i = 0; i < n; i += 2) sfx.push({ at: seg.speakAt + (10 + i * 2) / fps, cue: "count-blip" });
    }
    // The network graph's units locking on.
    if (seg.product === "pswitch") for (let i = 0; i < 4; i++) sfx.push({ at: seg.speakAt + (12 + i * 9) / fps, cue: "net-lock" });
  }
