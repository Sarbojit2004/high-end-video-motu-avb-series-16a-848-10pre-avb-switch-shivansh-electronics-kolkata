...
export const Outro: React.FC<{ canvas: Canvas }> = ({ canvas }) => {
  ...
  const Strip = (
    ...
    >
      {(["p16a", "p848", "p10pre", "pswitch"] as const).map((k, i) => (
        <React.Fragment key={k}>
          {i ? <div style={{ width: 12, height: 12, borderRadius: 6, background: "rgba(255,246,233,0.35)" }} /> : null}
          <div style={{ fontSize: S(k === "pswitch" ? 62 : 88), letterSpacing: 6, color: ACCENT[k].glow, textShadow: "0 6px 14px rgba(0,0,0,0.9)", whiteSpace: "nowrap" }}>
            {ACCENT[k].short}
          </div>
        </React.Fragment>
      ))}
