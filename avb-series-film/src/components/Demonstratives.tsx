...
export const IO: Record<"p16a" | "p848" | "p10pre", IoSpec> = {
  p16a: { mic: 0, line: 16, out: 16, phones: 1, label: "16 LINE IN · 16 LINE OUT" },
  p848: { mic: 4, line: 8, out: 12, phones: 2, label: "4 MIC + 8 LINE IN · 12 OUT" },
  p10pre: { mic: 10, line: 0, out: 8, phones: 2, label: "10 MIC IN · 8 LINE OUT" },
};
const SLOTS = 16;
 ...
 */
export const IoLadder: React.FC<{ product: "p16a" | "p848" | "p10pre"; f: number; width: number; s?: number }> = ({ product, f, width, s = 1 }) => {
  const { fps } = useVideoConfig();
  const accent = ACCENT[product];
 ...
 */
export const NetworkGraph: React.FC<{ f: number; width: number; s?: number; product?: ProductKey }> = ({ f, width, s = 1, product = "pswitch" }) => {
  const { fps } = useVideoConfig();
  const accent = ACCENT[product];
  const head = spring({ frame: f, fps, config: { damping: 200, mass: 0.5 }, durationInFrames: 14 });
  const nodes: { key: ProductKey; label: string; x: number }[] = [
    { key: "p10pre", label: "10pre · STAGE", x: 0.08 },
    { key: "p848", label: "848 · CONTROL ROOM", x: 0.36 },
    { key: "p16a", label: "16A · FRONT OF HOUSE", x: 0.64 },
    { key: "shared", label: "COMPUTER", x: 0.92 },
  ];
  const H = 250 * s;
  const cx = 0.5;
  const pulse = ((f % 48) / 48);
...
export const LadderCompare: React.FC<{ f: number; width: number; s?: number }> = ({ f, width, s = 1 }) => {
  const { fps } = useVideoConfig();
  const keys = ["p16a", "p848", "p10pre"] as const;
  const colGap = 26 * s;
  const colW = (width - colGap * 2) / 3;
  const cellGap = 5 * s;
  const cellW = (colW - cellGap * 15) / 16;
  const head = spring({ frame: f, fps, config: { damping: 200, mass: 0.5 }, durationInFrames: 14 });
  return (
    <div style={{ width, fontFamily: FONT.display }}>
            ...
            <div key={k} style={{ width: colW, opacity: colIn, transform: `translateY(${interpolate(colIn, [0, 1], [30, 0])}px)` }}>
              ...
              <div style={{ display: "flex", gap: cellGap }}>
                {Array.from({ length: 16 }).map((_, i) => {
                  const filled = i < total;
                  const lit = spring({ frame: f - 18 - ci * 10 - i * 1.5, fps, config: { damping: 200, mass: 0.35 }, durationInFrames: 10 });
