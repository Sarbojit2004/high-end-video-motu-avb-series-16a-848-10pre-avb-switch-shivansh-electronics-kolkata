...
export const Subhead: React.FC<{
  ...
  delay?: number;
  maxWidth?: number;
}> = ({ children, size = 40, color = COLORS.slate, delay = 16, maxWidth = 1180 }) => (
...
export const Editorial: React.FC<{
  ...
  maxWidth?: number;
  italic?: boolean;
}> = ({ children, size = 92, color = COLORS.ink, delay = 8, maxWidth = 1440, italic = false }) => (
...
export const Counter: React.FC<{
  ...
  size?: number;
  color?: string;
}> = ({ to, suffix = "", prefix = "", start = 8, len = 46, size = 96, color = COLORS.ink }) => {
  const frame = useCurrentFrame();
  return (
  ...
  <Rise delay={delay} y={18}>
    <div style={{ display: "flex", gap: 24, alignItems: "flex-start" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        {body ? <div style={{ ...subhead(29), color: COLORS.slate, maxWidth: 900 }}>{body}</div> : null}
      </div>
    </div>
  </Rise>
);
