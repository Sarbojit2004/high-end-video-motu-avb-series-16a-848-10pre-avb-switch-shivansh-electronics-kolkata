...
for (const { b, i } of targets) {
  ...
  const file = resolve(OUT, `${String(i).padStart(3, "0")}-ch${b.ch}-${b.id}.png`);
  await renderStill({
    composition,
    serveUrl,
    output: file,
  ...
  const { data, info } = await sharp(file).resize(192, 108, { fit: "fill" }).greyscale().raw().toBuffer({ resolveWithObject: true });
  const W = info.width, H = info.height, M = 5;
  ...
  report.push({ id: b.id, ch: b.ch, kind: b.kind, at, mean, ground });
  const flag = ground < 0.85 ? "  <-- GROUND NOT LIGHT" : "";
  console.log(
    `  ${String(i).padStart(3)} ch${b.ch} ${b.id.padEnd(22)} ${b.kind.padEnd(13)} ` +
