...
function rj45Snap() {
  const x = buf(0.20);
  mix(x, deClick(normalize(hp(noise(0.010, 77), 1800), 0.9), 1), 0.85, 0);
...
/** gPTP nanosecond-sync chime — two clean partials a fifth apart, locking. */
function gptpSync() {
  const x = buf(0.95);
  partial(x, 1568, 6.0, 0.5);
  mix(x, (() => { const y = buf(0.8); partial(y, 2349, 6.5, 0.42); return deClick(y, 3); })(), 1, 0.10);
  partial(x, 3136, 9, 0.14);
